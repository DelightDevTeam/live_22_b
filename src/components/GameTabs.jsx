        ...
        <div className="px-2 pt-0 mt-0 w-full" >
           ...
           {tabs.map((tab,index)=>{
                return <div onClick={()=>{
                    ...
                    navigate('/')
                 }} key={index} className={`py-2 px-2 px-sm-3 px-sm-4 gameTab cursor-pointer rounded-3 d-flex flex-column align-items-center justify-content-center  ${tab.value===selectedTab ? 'bgActive':  'bg-gradient'} `}>
                    <img src={tab.img} className='gameTabImg' />
                    <p className="gameText  ">{tab.name}</p>
                </div>
            })}
            <button onClick={onGameClick} className="mt-2 w-max py-3 px-5 h-max text-white demoPlay w-full rounded-5">
